...
public:
    // [15 pts] Inicialización: cada nodo es su propia raíz
    DisjointSet(int n) {
        parent.resize(n);
        rank.resize(n, 0);
        for (int i = 0; i < n; i++) {
    ...
    // [25 pts] Find con compresión de caminos (recursivo)
    int find(int x) {
        if (parent[x] != x) {
    ...
    // Opcional: mostrar estructura interna
    void showStructure() {
        cout << "Padres: ";
...
int main() {
    ...
    // Extra: mostrar estructura interna
